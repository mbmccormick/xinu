...
shellcmd xsh_udpdump(int nargs, char *args[])
{
	...
	uint32	remip, locip;		/* variables to hold the info	*/
	...
	printf("%5s %5s    %9s    %8s    %9s    %8s %3s %4s\n",
		"Entry", "State", "Remote IP", "Rem Port", "Local IP",
		"Loc Port", "Pid", "Pkts");
	printf("%5s %5s %15s %8s %15s %8s %3s %4s\n",
		"-----", "-----", "---------------", "--------",
		"---------------", "--------", "---", "----");
	...
	for (i = 0; i < UDP_SLOTS; i++) {
	    ...
	    state = uptr->udstate;
	    printf(
	    "%3d   %4s %3d.%3d.%3d.%3d %5d    %3d.%3d.%3d.%3d %6d%5d%6d\n",
	      i, udpstate[state], r1, r2, r3, r4, remprt, l1, l2,
	      l3, l4, locprt, pid, uptr->udcount);
	}
	return 0;
}
